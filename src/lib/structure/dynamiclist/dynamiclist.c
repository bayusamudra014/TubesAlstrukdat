...
void dl_free_list(DynamicList* dl) { free(dl->buffer); }
...
int dl_length(DynamicList dl) { return dl.nEff; }

/* Menghitung kapasitas dynamic list */
int dl_capacity(DynamicList dl) { return dl.capacity; }

/* Mengembalikan true bila list statis kosong */
boolean dl_is_empty(DynamicList dl) { return dl.nEff == 0; }

/* Mengembalikan true bila list statis penuh */
boolean dl_is_full(DynamicList dl) { return dl.nEff == dl.capacity; }
